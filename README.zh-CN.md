...
- 总调度 Agent：单角色出图请求会创建一个正式角色任务；同时可分析多角色合集原画、拆分单体角色，并把目标委派给各任务的专属 Asset Agent。
...
本仓库中的软件采用 [PolyForm Noncommercial License 1.0.0](./LICENSE) 许可。依照许可证条款，可以将其用于非商业目的并进行修改和再分发；任何商业用途均须事先获得版权所有者的单独书面授权。

第三方模型、服务、工作流和示例资产仍受其各自条款约束。软件许可证不授予 Super Idol Master 名称和 Logo 的商业使用权。
