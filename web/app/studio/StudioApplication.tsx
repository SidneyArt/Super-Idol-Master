...
import { CSSProperties, DragEvent as ReactDragEvent, FormEvent, KeyboardEvent as ReactKeyboardEvent, lazy, PointerEvent as ReactPointerEvent, Suspense, UIEvent as ReactUIEvent, useCallback, useEffect, useMemo, useRef, useState } from "react";
...
import { useCoordinatorState } from "./features/coordinator/useCoordinatorState";
import {
  PROCESS_KINDS,
  type AgentApiDraft,
  ...
  settingsDraft,
  useSettingsState,
} from "./features/settings/useSettingsState";
import { useTaskAgentState } from "./features/task-agent/useTaskAgentState";
...
export default function Studio({ initialRunId, initialWorkspaceId: requestedWorkspaceId, initialNotificationId, initialRuns, initialWorkspaces }: StudioProps) {
  ...
  }, [dispatcherForceScrollRef, setDispatcherScrollRequest]);
  ...
  }, [chatForceScrollRef, setChatScrollRequest]);
  ...
  const clearTaskConversation = useCallback(() => {
    setChatMessages([]);
    setChatSessionId("");
    setChatSessions([]);
    setChatContext(null);
  }, [setChatContext, setChatMessages, setChatSessionId, setChatSessions]);

  const clearDispatcherConversation = useCallback(() => {
    setDispatcherMessages([]);
  ...
  async function refreshActivity(showToast = false, signal?: AbortSignal) {
    ...
    const [controls, notificationData, generationData, batchData] = await Promise.all([
      ...
      api<{ generations: DispatcherGeneration[] }>(`/api/dispatcher/generations?workspaceId=${encodeURIComponent(workspaceId)}&sessionId=${encodeURIComponent(sessionId)}`, { signal }),
      api<{ batches: DispatcherTaskBatch[] }>(`/api/dispatcher/task-batches?workspaceId=${encodeURIComponent(workspaceId)}&sessionId=${encodeURIComponent(sessionId)}`, { signal }),
    ]);
    if (
      signal?.aborted
      || workspaceId !== selectedWorkspaceIdRef.current
      || runId !== selectedIdRef.current
      || sessionId !== dispatcherSessionIdRef.current
    ) return;
  ...
  usePollingQuery({
    ...
    enabled: Boolean(selectedWorkspaceId) && !dispatcherBusy,
    foregroundMs: 3_000,
    backgroundMs: 30_000,
    query: (workspaceId, signal) => api<ConversationPayload>(
      `/api/dispatcher/messages?workspaceId=${encodeURIComponent(workspaceId)}`,
      { signal },
    ),
    onData: (data, workspaceId) => {
      if (selectedWorkspaceIdRef.current !== workspaceId) return;
      if (
        dispatcherSessionIdRef.current
        && data.sessionId !== dispatcherSessionIdRef.current
      ) return;
      applyDispatcherConversation(data, dispatcherMessages.length === 0);
    },
    onError: (reason, workspaceId) => {
      if (
        selectedWorkspaceIdRef.current === workspaceId
        && dispatcherMessages.length === 0
      ) {
        setError(reason instanceof Error ? reason.message : "总调度对话读取失败");
      }
    },
  });
  ...
  useEffect(() => {
    ...
    approvalScrollKey,
    dispatcherBusy,
    dispatcherForceScrollRef,
    dispatcherGenerationScrollKey,
    dispatcherMessages,
    dispatcherNearBottomRef,
    dispatcherScrollRequest,
    dispatcherThreadRef,
  ]);
  ...
  usePollingQuery({
    ...
    foregroundMs: selectedTaskIsRunning || selectedRoleIsRunning || selectedPlanIsRunning
      ...
      : 10_000,
    backgroundMs: 30_000,
    query: async (runId, signal) => {
      const [runDetail, conversation] = await Promise.all([
        api<RunDetail>(`/api/runs/${runId}`, { signal }),
        api<ConversationPayload>(`/api/runs/${runId}/agent/messages`, { signal }),
      ]);
      return { runDetail, conversation };
    },
    onData: ({ runDetail, conversation }, runId) => {
      if (selectedIdRef.current !== runId) return;
      const changedRun = detail?.run.id !== runId;
      setDetail(runDetail);
      applyTaskConversation(conversation, changedRun);
      if (changedRun) {
  ...
  async function sendDispatcherMessage(event: FormEvent) {
    ...
    const optimistic: ChatMessage = {
      id: --dispatcherOptimisticIdRef.current, role: "user", content: message || "请分析这张角色合集原画并拆分任务。",
          ...
          <form className="chat-composer" onSubmit={sendChatMessage}>
            ...
            <div className="composer-footer">
              ...
              <div className="composer-actions">
                <button type="button" className="plus" onPointerDown={(event) => { event.preventDefault(); if (!run) { setError("请先选择任务，再向 Agent 消息中添加参考图片"); return; } try { if (!openAgentFilePicker()) setError("无法打开文件选择器，请刷新页面后重试"); } catch (err) { console.error("[Agent] 触发文件选择器失败", err); setError("无法打开文件选择器，请刷新页面后重试"); } }} onClick={(event) => event.preventDefault()} title="添加参考图片" aria-label="添加参考图片"><Plus size={17} /></button>
