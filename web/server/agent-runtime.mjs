...
import { createTaskAgentConversation } from "./features/agents/task-agent-conversation.mjs";
...
export function createAssetAgentRuntime({
  ...
  const conversation = createTaskAgentConversation({
    db,
    getPermissionMode,
    getRunDetail,
  ...
  async function repairQaAndRegenerate(runId, failureReason) {
    ...
    const blockRepair = (operation, error) => {
      const detail = error instanceof Error ? error.message : "未知错误";
      const message = `${operation}未能启动：${detail}`;
      updateWorkflowPlan(runId, "blocked", message);
      addMessage(runId, "assistant", `自动修复已暂停：${message}`);
      return getWorkflowPlan(runId);
    };
    ...
    if (deterministicRepair?.strategy === "stale_source") {
      return blockRepair("确定性修复", new Error(deterministicRepair.reason));
    }
    if (deterministicRepair?.applied) {
      ...
      try {
        ...
        return blockRepair("确定性修复后的 SDPose 复检", error);
      }
    }
    ...
    try {
      ...
      try {
        await prepareCharacterPrompts(
          ...
          null,
          { preserveTposeRepairSource: true },
        );
      } catch (error) {
        updatePrompts(runId, {
          ...candidate,
      ...
      return blockRepair("图片编辑模型修复", error);
    }
  }
