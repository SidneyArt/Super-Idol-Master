# Super-Idol-Master 多智能体数字角色资产生产线 PRD
...
Super-Idol-Master（多智能体数字角色资产生产线）是一个面向独立游戏开发者、虚拟内容创作者和轻量 3D 团队的对话式角色资产工作台。用户不需要理解 ComfyUI 节点、图生 3D 参数或自动绑骨流程，只需描述角色、选择候选图并在关键节点确认，即可完成：
...
| Mixamo 动画重定向与预览 | 已完成的增强能力 | 已用 SkinTokens 绑定 GLB 和 Mixamo Idle／Walking 动画验证浏览器内重定向与播放；不写回 GLB |
...
- 对已绑定角色支持导入和复用 Mixamo FBX 动画，并提供播放、暂停、时间轴、循环、速度和原地移动预览；关闭或复位后恢复原始 Pose，不写回资产。
...
- Mixamo 动画可重定向到已绑定角色，并支持播放、暂停、时间轴和静态姿态复位。
- 任何预览编辑都只存在于内存中的场景副本。
...
- 对未经验证的第三方骨骼或所有 Mixamo 动画宣称完全兼容。
- 未测试的准确率和性能提升。
- 把普通提示词模板包装成模型微调。
...
| 骨骼命名或绑定姿态与 Mixamo 不完全兼容 | 中/中 | 导入时检查核心骨骼；基于源／目标绑定姿态重定向；保留静态姿态复位；逐类动画验收 |
